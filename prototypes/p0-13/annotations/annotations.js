(function () {
  ...
  window.AnnotationData = {
    ...
    'activity-combo': [
      sourceNote('组合活动','SIT 组合活动列表/新增；Axure 1.5；2026-08-10 已确认展示名称与适用门店增量；2026-08-20 用户确认门店采用覆盖导入。'),
      ...
      note(5,'[data-anno="combo-store-selector"]','可用门店选择器','门店树选择','按品牌、省、市分组展示可用门店，支持搜索、已选查看和导入。','门店主数据可用记录。','分组或单店勾选，确认回填数量与名称；导入另行进入覆盖面板。','暂停营业、已停业等不可用门店不进入列表。','搜索无结果时显示空态，不展示营业状态标签。'),
      note(6,'[data-anno="combo-store-import"]','门店覆盖导入','批量替换适用门店','通过文件批量形成新的门店选择结果，且仅允许覆盖当前已选门店。','2026-08-20 用户确认覆盖导入；校验数据为 Mock。','选择文件后展示演示校验，确认覆盖后回到已选门店；取消不改变原选择。','覆盖结果只保留可用门店，不追加到旧选择。','正式模板字段、文件限制、错误明细和接口处理待确认。')
    ],
